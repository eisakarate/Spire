...
with open(path.join(startup_dir, f"omp_{file_suffix}.json"), "w") as fw:
    fw.write(jData)
print("One-Mode-Projection Result")
#output helpful json for drawing maps in LATEX
for cur_n in projection.nodes: 
    for cur_e in cur_n.get_edge_list():
        print(f"({projection.nodes[cur_e.from_node].label}, {projection.nodes[cur_e.to_node].label}) - Weight: {cur_e.weight}")
